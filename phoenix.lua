...
v.Name = "SuperEvadeB"
if game.ReplicatedStorage.Styles.Brawler:FindFirstChild("MoveName") then
	v = game.ReplicatedStorage.Styles.Brawler["MoveName"]
else
	v = Instance.new("StringValue", game.ReplicatedStorage.Moves)
end
...
v.Value = Color3.fromRGB(3, 227, 252)
v.Name = "Color"
...
Main.HeatMove.TextLabel:GetPropertyChangedSignal("Text"):Connect(function()
    if Main.HeatMove.TextLabel.Text == "Ultimate Essence" then
		Main.HeatMove.TextLabel.Text = Rep.Moves.MoveName.Value
...
--[[Credits to MisterMax228 for helping me out on this

    Right 2 Fight Combat Voice Mod by Mark512345
        - This mod lets you play the voicelines that are available in the game (Kiryu, Akiyama, Majima and Vulcan) during combat.
        - The voicelines can play while doing light attacks, heavy attacks, dodging, heat actions, etc.
        - If you have any queries, suggestions or anything relating to this mod, let me know!
        
    Note: You can switch between character voices by pressing H. This mod will be updated.
    ]]
...
local function sendNotification(text, color, sound)
	...
	pgui["नोटिफ"]:Fire(text, sound or nil)
end
...
for i,v in pairs(styles:GetChildren()) do
	if v:FindFirstChild"VisualName" and v.VisualName.Value == "Phoenix" and v.Name ~= "Brawler" then
		kiryu = v
		break
	end
end
...
status.Style.Changed:Connect(style_changed)
	...
	if game.UserInputService:GetFocusedTextBox() == nil then
		...
		elseif key.KeyCode == Enum.KeyCode.Z then
			if not dashcooldown then
				...
				local b2 = game:GetService("RunService").Heartbeat:Connect(function(d)
					root.CFrame += (bodyvel.Velocity * d) * 4
				end)
...
main.Heat.Fill.ImageColor3 = Color3.fromRGB(3, 177, 252)
main.Heat.Fill2.ImageColor3 = Color3.fromRGB(3, 103, 252)
main.Heat.ClimaxFill.ImageColor3 = Color3.fromRGB(180, 58, 174)
main.Heat.ClimaxFill2.ImageColor3 = Color3.fromRGB(255, 14, 199)